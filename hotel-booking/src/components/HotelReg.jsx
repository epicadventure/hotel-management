...
const HotelReg = () => {
    ...
    const onSubmitHandler = async (e) => {
        try {
            ...
            const { data } = await axios.post('/api/hotel', { name, contact, address, city }, { headers: { Authorization: `Bearer ${token}` } })
    ...
    return (
        <div onClick={() => setShowHotelReg(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/70'>
            <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className='flex bg-white p-8 rounded-xl max-w-4xl max-md:mx-2'>
                ...
                <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
                    ...
                    <div className='w-full mt-4 max-w-60 mr-auto'>
                        ...
                        <select onChange={(e) => setCity(e.target.value)} value={city} className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light cursor-pointer' id='city' required>
